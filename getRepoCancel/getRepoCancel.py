    ...
    try:
        ...
        if lastItem == '_':
            ...
            if providerId == '_' and locationId != '_':
                response = dynamodb.query(
                    ...
                    ExpressionAttributeValues={
                        ':key': {'S': 'BUS#' + businessId + '#LOC#' + locationId + '#5'},
                        ':dateIni': {'S': dateIni},
                        ':dateFin': {'S': dateFin}
                    },
                    Limit=10
            ...
            if providerId != '_':
                response = dynamodb.query(
                    ...
                    ExpressionAttributeValues={
                        ':key': {'S': 'BUS#' + businessId + '#LOC#' + locationId + '#PRO#' + providerId + '#5'},
                        ':dateIni': {'S': dateIni},
                        ':dateFin': {'S': dateFin}
                    },
                    Limit=10
        ...
        for row in json_dynamodb.loads(response['Items']):
            ...
            recordset = {
                ...
                'Cancel': str(row['TIMECANCEL'])[0:10] if 'TIMECANCEL' in row else '',
                'Priority': priority
            }
