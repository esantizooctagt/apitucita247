    ...
    try:
        ...
        locs = dynamodb.query(
            ...
            ExpressionAttributeValues = {
                ...
                ':key': {'S': 'LOC#' if locationId == '0' else 'LOC#' + locationId},
                ':stat' : {'N': '1'}
            }
        )
                                ...
                                for currDate in json_dynamodb.loads(getCurrDate['Items']):
                                    ...
                                    for cancel in json_dynamodb.loads(getCurrHours['Items']):
                                        if int(cancel['CANCEL']) == 1:
                                            ...
                                            timeExists = findTime(int(cancel['SKID'].replace('HR#','').replace('-','')), hoursBooks)
                                            if timeExists == '':
                                                hoursBooks.append(recordset)
                                            else:
                                    ...
                                    for item in hoursBooks:
                                        # logger.info(item)
                                        if item['Cancel'] == 1:
                                            timeExists = findTime(str(item['Hour']).rjust(4,'0')[0:2]+':'+str(item['Hour']).rjust(4,'0')[-2:], hoursData)
                                            if timeExists != '':
                                            ...
                                            if (int(item['TimeService']) > 15):
                                                ...
                                                if count == -1:
                                                    count = custPerTime          
                                                recordset = {
                                                    'Hour': str(item['Hour']).rjust(4,'0')[0:2]+':'+str(item['Hour']).rjust(4,'0')[-2:], #newTime,
                                                    'Time24': int(item['Hour']), #time24hr,
                                    ...
                                    for h in workHours():
                                        ...
                                        if found == '':
                                            ...
                                            for item in dateAppo:
                                                ...
                                                # logger.info('Dt ' + str(h) + ' hr: ' + hStd + ' addVal ' + str(times[addVal]) + ' -- ini: ' + str(ini) + ' -- fin: ' + str(fin))
                                                            ...
                                                            if citas > 0:
                                                                ...
                                                                if getApp == '':
                                                                    ...
                                                                    for item02 in dateAppo:
                                                                        ...
                                                                        if int(nextHr) >= ini02 and int(nextHr) <= fin02:
                                                                            entro = 1
                                                                            break
                                                                    if entro == 0:
                                                                        count = 0
